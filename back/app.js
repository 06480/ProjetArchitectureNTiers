...
mongoose
  ...
  .then((r) => {
    ...
    app.post("/baskets/create", jsonParser, (req, res) => {
      let user = new User({
        _id: mongoose.Types.ObjectId(),
        beersId: req.body.beersId,
        userId: req.body.userId,
      });
      user.save((err, basket) => {
        console.log(basket);
      });

      res.json();
    });
    app.delete("/baskets/delete/:id", (req, res) => {
      Basket.findByIdAndDelete(req.params.id, (err, user) => {
        console.log(err);
      });
    });
    app.patch("/baskets/change/:id", jsonParser, (req, res) => {
      Basket.findByIdAndUpdate(req.params.id, { beersId: req.body.beersId, userId: req.body.userId }, (err, user) => {});
      res.status(204).send();
    });
